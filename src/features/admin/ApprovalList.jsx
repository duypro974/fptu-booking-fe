...
export default function ApprovalList() {
  ...
  const isFacilityAdmin =
    user?.role === "facility_admin" || user?.role === "FACILITY_ADMIN";
  ...
  const isRecurringBooking = (booking) => {
    const hasBookingGroupId =
      booking.bookingGroupId !== null && booking.bookingGroupId !== undefined;
    const hasBookingsArray =
      Array.isArray(booking.bookings) && booking.bookings.length > 0;
  ...
  useEffect(() => {
    ...
    if (user?.campus || user?.campusId) {
      ...
      console.warn(
        "[ApprovalList] User không có campus hoặc campusId, không thể load data"
      );
      setLoading(false);
    }
  }, [user]);
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      if (!campus) {
        console.warn(
          "[ApprovalList.loadRequests] Không thể xác định campus, dùng mặc định 'hcm'"
        );
        campus = "hcm";
      ...
      console.log(
        "[ApprovalList.loadRequests] Loading approvals for campus:",
        campus
      );
      ...
      console.log(
        "[ApprovalList.loadRequests] Received pending data:",
      ...
      try {
        ...
        console.log(
          "[ApprovalList.loadRequests] Received all bookings:",
          allBookingsData?.length || 0,
          "bookings"
        );
      } catch (error) {
        console.warn(
          "[ApprovalList.loadRequests] Error loading all bookings (may not be available):",
          error
        );
      ...
      console.log(
        "[ApprovalList] Total APPROVED items from getAllBookings:",
      ...
      if (item23) {
        console.log("[ApprovalList] Item 23 (known recurring) in APPROVED:", {
          ...
          hasBookingsArray:
            Array.isArray(item23.bookings) && item23.bookings.length > 0,
      ...
      const itemsWithGroupId = approvedItems.filter(
        (item) =>
          item.bookingGroupId !== null && item.bookingGroupId !== undefined
      );
      console.log(
        "[ApprovalList] APPROVED items with bookingGroupId:",
        itemsWithGroupId.length
      );
      if (itemsWithGroupId.length > 0) {
        // Group by bookingGroupId to find parent recurring bookings
        const groupIds = [
          ...new Set(itemsWithGroupId.map((item) => item.bookingGroupId)),
        ];
        console.log(
          "[ApprovalList] Unique bookingGroupIds in APPROVED:",
          groupIds
        );

        // For each group, check if there's a parent booking
        groupIds.forEach((groupId) => {
          const parentBooking = allBookingsData.find(
            (item) => item.id === groupId
          );
          if (parentBooking) {
            console.log(
              "[ApprovalList] Parent booking for groupId",
              groupId,
              ":",
              {
                id: parentBooking.id,
                ...
                hasBookingsArray:
                  Array.isArray(parentBooking.bookings) &&
                  parentBooking.bookings.length > 0,
      ...
      if (approvedItems.length > 0) {
        console.log("[ApprovalList] Sample APPROVED item:", {
          ...
          hasBookingsArray:
            Array.isArray(approvedItems[0].bookings) &&
            approvedItems[0].bookings.length > 0,
      ...
      console.log(
        "[ApprovalList] Filtered approved recurring child slots:",
        approvedRecurring.length
      );
      ...
      console.log(
        "[ApprovalList] Grouped into",
        Object.keys(groupedByParent).length,
        "parent groups"
      );

      // Aggregate each group into a single parent booking object
      const aggregatedParents = Object.entries(groupedByParent).map(
        ([groupId, childSlots]) => {
          // Use the first child slot as base
          const firstSlot = childSlots[0];

          // Calculate date range: min(startTime) to max(endTime)
          const startTimes = childSlots
            .map((slot) =>
              slot.startTime ? new Date(slot.startTime).getTime() : 0
            )
            .filter((t) => t > 0);
          const endTimes = childSlots
            .map((slot) =>
              slot.endTime ? new Date(slot.endTime).getTime() : 0
            )
            .filter((t) => t > 0);
          const minStartTime =
            startTimes.length > 0
      ...
      console.log(
        "[ApprovalList] Aggregated parents:",
        aggregatedParents.length
      );
      ...
      const uniqueApproved = aggregatedParents.filter(
      ...
      console.log(
        "[ApprovalList.loadRequests] Combined data:",
        data.length,
        "requests (pending:",
        pendingData?.length || 0,
        "approved recurring:",
      ...
      console.log(
        "[ApprovalList.loadRequests] Mapped and sorted data:",
        sortedData
      );
      console.log(
        "[ApprovalList.loadRequests] Sorted data:",
        sortedData.length,
        "requests"
      );
      ...
      console.error(
        "[ApprovalList.loadRequests] Lỗi tải danh sách yêu cầu:",
        error
      );
  ...
  const getSlotsFromTimeRange = (startTime, endTime) => {
    ...
    if (!startSlot) {
      if (startHours < 7) startSlot = 1;
        ...
        startSlot = Math.min(
          5,
          Math.max(1, Math.floor((startHours - 7) / 2) + 1)
        );
    }
    ...
    if (!endSlot) {
      if (endHours <= 7) endSlot = 1;
        ...
        const calculatedSlot = Math.min(
          5,
          Math.max(1, Math.floor((endHours - 7) / 2) + 1)
        );
  ...
  const fetchAvailableRooms = async (startTime, endTime, currentRoomId) => {
    ...
    try {
      ...
      console.log(
        "[fetchAvailableRooms] ========== BẮT ĐẦU TÌM PHÒNG TRỐNG =========="
      );
      console.log("[fetchAvailableRooms] Input:", {
        ...
        slots: slots,
        slotRanges: slots.map(
          (s) =>
            `Slot ${s}: ${SLOT_MAPPING[s]?.start || 0}:00 - ${
              SLOT_MAPPING[s]?.end || 0
            }:00`
        ),
      });
      ...
      try {
        ...
        for (const slot of slots) {
          try {
            console.log(
              "[fetchAvailableRooms] Gọi API searchAvailableRooms với:",
              { date, slot }
            );
            ...
            console.error(
              "[fetchAvailableRooms] Lỗi khi gọi API cho slot",
              slot,
              ":",
              slotError
            );
        ...
        if (roomSetsBySlot.length > 0) {
          ...
          for (let i = 1; i < roomSetsBySlot.length; i++) {
            ...
            allAvailableRooms = allAvailableRooms.filter((room) =>
              currentRoomIds.has(room.id)
            );
          }

          console.log(
            "[fetchAvailableRooms] ✅ Intersection của",
            slots.length,
            "slot:",
            allAvailableRooms.length,
            "phòng trống trong TẤT CẢ các slot"
          );
        }
        ...
        if (!allAvailableRooms || allAvailableRooms.length === 0) {
          console.warn(
            "[fetchAvailableRooms] ⚠️ Không có phòng trống trong TẤT CẢ các slot",
            slots
          );
          console.warn(
            "[fetchAvailableRooms] Thử fallback: Lấy tất cả phòng ACTIVE (không chắc chắn là trống)"
          );
        ...
        console.error(
          "[fetchAvailableRooms] ❌ API searchAvailableRooms failed:",
          searchError
        );
        console.error(
          "[fetchAvailableRooms] Error message:",
          searchError.message
        );
        ...
        if (
          ...
          searchError.message?.includes("permission")
        ) {
          console.warn(
            "[fetchAvailableRooms] ⚠️ API bị chặn quyền (403). Sử dụng fallback."
          );
        }

        console.log(
          "[fetchAvailableRooms] 🔄 Fallback: Lấy tất cả phòng ACTIVE của campus"
        );
        ...
        const campusId =
          user?.campusId ||
          (user?.campus === "hcm" ? 2 : user?.campus === "hn" ? 1 : 2);
        ...
        console.log(
          "[fetchAvailableRooms] Fallback - getRooms returned",
          allRooms?.length || 0,
          "rooms"
        );
        ...
        console.log(
          "[fetchAvailableRooms] ⚠️ Fallback filtered to",
          allAvailableRooms.length,
          "rooms (KHÔNG CHẮC CHẮN là trống)"
        );
      }
      ...
      const filteredRooms = (allAvailableRooms || []).filter((room) => {
        const roomId = room.id?.toString();
        const currentId = currentRoomId?.toString();
        const status = (room.status || room.facilityStatus || "")
          .toString()
          .toUpperCase();
        const facilityStatus = (room.facilityStatus || "")
          .toString()
          .toLowerCase();
      ...
      console.log(
        "[fetchAvailableRooms] ✅ Kết quả cuối cùng:",
        filteredRooms.length,
        "phòng trống"
      );
      console.log(
        "[fetchAvailableRooms] Danh sách phòng:",
        filteredRooms.map((r) => ({ id: r.id, name: r.name }))
      );
  ...
  const handleApprove = async (request) => {
    ...
    const currentRoomId =
      request.facilityId || request.roomId || request.facility?.id;
    ...
    const requestRoomStatus =
      request.facility?.status || request.room?.status || request.status;
    ...
    if (!roomMaintenance && !roomInactive && currentRoomId && isFacilityAdmin) {
      try {
        ...
        const roomStatusValue = String(
          roomDetail?.status || roomDetail?.facilityStatus || ""
        ).toUpperCase();
    ...
    try {
      ...
      console.error(
        "[handleApprove] Lỗi kiểm tra conflict (không block):",
        error
      );
      setConflictCount(0);
      setConflictList([]);
    ...
    const shouldFetchRooms =
      (conflicts.length > 0 || roomMaintenance || roomInactive) &&
      request.startTime &&
      request.endTime &&
      isFacilityAdmin;
    ...
    if (shouldFetchRooms) {
      try {
        ...
        console.error(
          "[handleApprove] Lỗi fetch available rooms (không block):",
          error
        );
      ...
      console.log(
        "[handleApprove] ⚠️ Không fetch available rooms vì điều kiện không thỏa"
      );
    }
  ...
  const confirmApprove = async () => {
    try {
      ...
      const alternativeFacilityId =
        (isFacilityAdmin && selectedNewRoom?.id) || null;
      ...
      const message = alternativeFacilityId
        ? `Đã duyệt đơn thành công và chuyển sang phòng ${
            selectedNewRoom.name || selectedNewRoom.facilityName
          }!${
            conflictCount > 0
              ? ` ${conflictCount} đơn trùng lịch đã bị tự động từ chối.`
              : ""
          }`
  ...
  const {
    singlePendingRequests,
    recurringPendingRequests,
    recurringActiveRequests,
  } = useMemo(() => {
    const singlePending = [];
    ...
    console.log(
      "[ApprovalList] Items with isGroup=true:",
      itemsWithIsGroup.length
    );
    ...
    requests.forEach((req, index) => {
      ...
      if (req.isGroup === true || index < 3) {
        console.log(`[ApprovalList] Request ${index}:`, {
          ...
          hasBookingsArray:
            Array.isArray(req.bookings) && req.bookings.length > 0,
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500">
              Xem và xử lý các yêu cầu đặt phòng đang chờ duyệt tại{" "}
              {user?.campusName}.
            </p>
          </div>
            ...
            {loading
              ...
              : (() => {
                  ...
                  if (activeTab === "RECURRING_PENDING")
                    return recurringPendingCount;
                  if (activeTab === "RECURRING_ACTIVE")
                    return recurringActiveCount;
                  return 0;
                })()}{" "}
              ...
              <div className="flex gap-1">
                ...
                >
                  ...
                  {singlePendingCount > 0 && (
                    <Badge
                      type={
                        activeTab === "SINGLE_PENDING" ? "warning" : "secondary"
                      }
            ...
            {filteredRequests.map((req, index) => (
              <Card
                ...
                className={`p-6 hover:shadow-md transition-shadow cursor-pointer ${
                  index === 0 ? "border-2 border-green-200 bg-green-50/30" : ""
                }`}
                onClick={() => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900">
                          ...
                          req.participantCount !== undefined &&
                      ...
                      <div className="pt-4 border-t border-gray-100">
                        <div className="space-y-2">
                          ...
                          <div>
                            ...
                            {req._aggregated.minStartTime &&
                            req._aggregated.maxEndTime ? (
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-gray-900">
                                {req._aggregated.usedSlots}/
                                {req._aggregated.totalSlots} buổi
                              </span>
                              <Badge type="success" className="text-xs">
                                Đang hoạt động
                              </Badge>
                      ...
                      <>
                        <Button
                          ...
                          onClick={(e) => {
                            ...
                            console.log(
                              "[ApprovalList] Opening detail modal for RECURRING_ACTIVE:",
                              {
                                itemId: req.id,
            ...
            <Card className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md max-h-[90vh] flex flex-col">
              ...
              <div className="mb-4 px-6 overflow-y-auto flex-1">
                <p className="text-sm text-gray-600 mb-2">
                  Bạn đang từ chối yêu cầu đặt phòng{" "}
                  <strong>{selectedRequest.roomName}</strong> của{" "}
            ...
            <Card className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg max-h-[90vh] flex flex-col">
              ...
              <div className="mb-4 px-6 overflow-y-auto flex-1">
                <p className="text-sm text-gray-600 mb-4">
                  Bạn đang duyệt yêu cầu đặt phòng{" "}
                  <strong>{selectedRequest.roomName}</strong> của{" "}
                  <strong>{selectedRequest.userName}</strong>.
                </p>
                ...
                {conflictCount > 0 || isRoomMaintenance || isRoomInactive ? (
                  <div className="space-y-4">
                    ...
                    {isRoomMaintenance && (
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        ...
                        <p className="text-sm text-yellow-800">
                          Phòng <strong>{selectedRequest.roomName}</strong> đang
                          trong trạng thái <strong>Bảo trì</strong>. Bạn nên
                          chuyển đơn sang phòng khác.
                        </p>
                      </div>
                    )}
                    ...
                    {isRoomInactive && (
                      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                        ...
                        <p className="text-sm text-red-800">
                          Phòng <strong>{selectedRequest.roomName}</strong> đang
                          trong trạng thái <strong>Ngừng hoạt động</strong>. Bạn
                          nên chuyển đơn sang phòng khác.
                        </p>
                      </div>
                    )}
                    ...
                    {conflictCount > 0 && (
                      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                        ...
                        <p className="text-sm text-red-800 mb-3">
                          Phòng <strong>{selectedRequest.roomName}</strong> đã
                          bị đặt trùng lịch với <strong>{conflictCount}</strong>{" "}
                          yêu cầu khác.
                        </p>
                        ...
                        <div className="space-y-2 max-h-32 overflow-y-auto">
                          {conflictList.map((conflict, idx) => {
                            const isOlder =
                              conflict.createdAt && selectedRequest.createdAt
                                ? new Date(conflict.createdAt).getTime() <
                                  new Date(selectedRequest.createdAt).getTime()
                                : false;
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    <span className="font-medium text-gray-900">
                                      #{conflict.bookingCode || conflict.id}
                                    </span>
                                    <span className="text-gray-600">
                                      - {conflict.userName || "N/A"}
                                    </span>
                                    {isOlder && (
                        ...
                        {isUsingFallback && (
                          <div className="mb-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg">
                            <p className="text-xs text-yellow-800">
                              ⚠️ <strong>Lưu ý:</strong> Đang hiển thị tất cả
                              phòng ACTIVE (không chắc chắn là trống trong khung
                              giờ này) vì không thể kiểm tra availability. Vui
                              lòng kiểm tra thủ công trước khi chuyển phòng.
                            </p>
                          </div>
                        )}
                          ...
                          >
                            ...
                            {availableRooms.map((room) => (
                              <option key={room.id} value={room.id}>
                                {room.name || room.facilityName} - Sức chứa:{" "}
                        ...
                        {selectedNewRoom && (
                          <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                            <p className="text-sm text-green-800">
                              ✅ Đã chọn:{" "}
                              <strong>
                                {selectedNewRoom.name ||
                                  selectedNewRoom.facilityName}
                              </strong>
                              {selectedNewRoom.capacity &&
                                ` (${selectedNewRoom.capacity} người)`}
                            </p>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      <p className="text-sm text-blue-800">
                        <strong>Lưu ý:</strong>
                        {conflictCount > 0 &&
                          ` Nếu bạn duyệt đơn này, ${conflictCount} yêu cầu trùng lịch sẽ tự động bị từ chối.`}
                        {isRoomMaintenance &&
                          " Phòng đang bảo trì, không thể sử dụng."}
                        {isRoomInactive &&
                          " Phòng đang ngừng hoạt động, không thể sử dụng."}
                        {isFacilityAdmin &&
                          selectedNewRoom &&
                          " Đơn này sẽ được chuyển sang phòng mới."}
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-green-800 mt-2">
                      Phòng <strong>{selectedRequest.roomName}</strong> còn
      ...
      <BookingActionModal
        ...
        onSuccess={async (result) => {
          ...
          try {
            ...
            console.error(
              "[ApprovalList] Error reloading requests after booking creation:",
              error
            );
      ...
      <BookingDetailModal
        ...
        onSuccess={async (message) => {
          try {
            ...
            console.error(
              "[ApprovalList] Error reloading requests after detail action:",
              error
            );
          } finally {
            setShowDetailModal(false);
            setSelectedBookingId(null);
            setSelectedBookingData(null);
            alert(message || "Duyệt thành công!");
          }
        }}
      />
    </AdminLayout>
